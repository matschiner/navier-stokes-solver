...
generic_H1_func = GridFunction(VH1)
generic_H1_func_transformed = GridFunction(X)
generic_H1_func.Set((x * x, y * y))
...
generic_H1_func_transformed.vec.data = E * generic_H1_func.vec

laplaceH1 = BilinearForm(VH1)
laplaceH1 += InnerProduct(grad(vH1trial), grad(vH1test)) * dx
laplaceH1.Assemble()

laplaceH1_inverse = laplaceH1.mat.Inverse(freedofs=VH1.FreeDofs())

Ahat_inv = E @ laplaceH1_inverse @ E.T
...
# preA = c.mat
preA = Ahat_inv # jacobi part still missing
